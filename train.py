...
def main():
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        print('eval psnr: {:.2f},eval ssim:{:.2f} eval loss :{:.2f}'.format(psnr, ssim, test_loss))
...
def train(
    ...
    msrn_model.train()
    with tqdm(total=batches*config.batch_size,ncols=100) as t:
        t.set_description('epoch: {}/{}'.format(epoch, config.epochs))
        ...
        while batch_data is not None:
            ...
            batch_index += 1
            t.set_postfix(trainloss='{:.6f}'.format(losses.avg))
            t.update(config.batch_size)
