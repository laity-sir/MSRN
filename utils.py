...
def save_checkpoint(
    ...
    # checkpoint_path = os.path.join(path, "model_epoch{}.pth".format(state_dict['epoch']))
    checkpoint_path = os.path.join(path, "model.pth")
    torch.save(state_dict, checkpoint_path)

    if is_best:
        shutil.copyfile(checkpoint_path, os.path.join(results_dir, "best.pth"))
    if is_last:
        shutil.copyfile(checkpoint_path, os.path.join(results_dir, "last.pth"))
...
class ProgressMeter(object):
    ...
    meters:需要显示的条目
    prefix：前缀
    使用：首先定义一个ProgressMeter类，类中必须声明进度的数量和需要打印的条目。然后多久显示需要调用display()函数
    """
